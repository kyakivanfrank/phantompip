...
export default function SubscriptionClient({ paymentDetails }: { paymentDetails: Record<PaymentMethod, PaymentDetail> }) {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <p className="text-gray-400">Select your preferred gateway to activate your 30-day trading access session.</p>
      </motion.div>

      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="space-y-6">
        <div className="grid gap-4 sm:grid-cols-3">
          {(Object.keys(paymentDetails) as PaymentMethod[]).map((method) => {
              ...
              >
                <div>
                  ...
                  <h3 className="font-semibold text-white flex items-center gap-2 text-base">
                    {IsCryptoTab ? <Coins className="h-5 w-5 text-cyan-400" /> : <Smartphone className="h-5 w-5 text-purple-400" />}
                    {details.label}
                  </h3>
                </div>
        ...
        <div className="rounded-xl border border-white/[0.1] bg-dark-secondary/40 backdrop-blur-xl overflow-hidden">
          <div className="p-6 border-b border-white/[0.1] bg-white/[0.01]">
            <h3 className="font-semibold text-white text-lg mb-4">Step 1: Send Payment via {currentDetails.label}</h3>
            ...
            <div className="grid gap-6 md:grid-cols-2 items-start">
              <div className="space-y-3">
                ...
                <div>
                  <p className="text-xs text-gray-400 uppercase tracking-wider mb-2">{isCrypto ? 'Wallet Address (TRON Network)' : 'Recipient Mobile Number'}</p>
                  <div className="flex gap-2">
                    <input type="text" value={currentDetails.address} readOnly className="flex-1 rounded-lg border border-white/[0.1] bg-dark-tertiary/50 px-3 py-2 font-mono text-sm text-gray-300 outline-none" />
                    <button type="button" onClick={() => copyToClipboard(currentDetails.address, formData.method)} className={`px-3 py-2 rounded-lg border transition-colors ${isCrypto ? 'border-cyan-500/30 bg-cyan-500/10 text-cyan-400 hover:bg-cyan-500/20' : 'border-purple-500/30 bg-purple-500/10 text-purple-400 hover:bg-purple-500/20'}`}>
                      {copied === formData.method ? <Check className="h-4 w-4" /> : <CopyIcon className="h-4 w-4" />}
                    </button>
                  </div>
                  <p className="text-xs text-gray-300 font-medium mt-1.5 bg-white/[0.03] inline-block px-2 py-0.5 rounded border border-white/[0.05]">{currentDetails.accountName}</p>
                </div>
              </div>
              ...
              <div className="space-y-3 p-4 rounded-xl bg-white/[0.02] border border-white/[0.05] text-sm text-gray-400">
                ...
                <p>Once transaction is confirmed by your network, capture the tracking reference ID or TxHash generated by the system.</p>
              </div>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div>
              ...
              <input type="text" id="transactionId" name="transactionId" value={formData.transactionId} onChange={handleChange} placeholder={isCrypto ? "e.g., 0x7a4d... or TRC20 TxHash" : "e.g., ID240101.1402.C00001 or Money Ref"} required className="mt-2 w-full rounded-lg border border-white/[0.1] bg-dark-tertiary/50 px-4 py-2.5 text-white placeholder:text-gray-500 outline-none focus:border-purple-500/50 transition-colors font-mono text-sm" />
            ...
            <button type="submit" disabled={isLoading} className={`w-full rounded-lg px-4 py-2.5 font-medium text-white transition-colors disabled:opacity-50 ${isCrypto ? 'bg-cyan-600 hover:bg-cyan-700' : 'bg-purple-600 hover:bg-purple-700'}`}>
              {isLoading ? (
